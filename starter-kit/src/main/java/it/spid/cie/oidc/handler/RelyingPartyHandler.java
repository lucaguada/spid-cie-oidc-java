...
import it.spid.cie.oidc.helper.*;
...
public class RelyingPartyHandler {
  ...
  public String getAuthorizeURL(
    ...
    if (!Validator.isNullOrEmpty(redirectUri)) {
      if (!Json.contains(entityRedirectUris, redirectUri)) {
        logger.warn(
  ...
  private WellKnownData prepareOnboardingData(String sub, boolean jsonMode)
    ...
    rpJson.put("client_registration_types", Json.array("automatic"));
    ...
    json.put("metadata", metadataJson);
    json.put(
      "authority_hints", Json.array(options.getDefaultTrustAnchor()));
